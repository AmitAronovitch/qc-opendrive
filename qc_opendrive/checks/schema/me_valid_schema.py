...
class MESchemaValidator(SchemaValidator):
    ...
    @abstractmethod
    def _get_error_severity(self, schema_error: SchemaValidator.SchemaError) -> IssueSeverity:
        ...
        for element in elements:
            ...
            if element.get('fillType') == 'trafficIsland' and 'trafficIsland' in schema_error.message and 'trafficIsland' in schema_error.message:
                continue

            # Ignore object lengths of 0 errors
            if element.tag == 'object' and \
                schema_error.message.startswith("Element 'object', attribute 'length': [facet 'minExclusive'] The value ") and \
                schema_error.message.endswith(" must be greater than '0.0'."):
            ...
            if element.tag == 'line' and \
                schema_error.message.startswith("Element 'line', attribute 'sOffset': [facet 'minInclusive'] The value ") and \
                schema_error.message.endswith(" is less than the minimum value allowed ('0.0')."):
