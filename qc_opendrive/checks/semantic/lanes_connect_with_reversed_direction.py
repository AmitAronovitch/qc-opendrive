...
def _check_road_lanes(
    ...
    lanes = set(
        [l for l in utils.get_lanes(lane_sections[-1]) if l.get("id") < "0"]
        + [l for l in utils.get_lanes(lane_sections[0]) if l.get("id") > "0"]
    )
    ...
    for lane in lanes:
        ...
        for connected_lane, connected_road in connected_lanes_with_roads:
            ...
            id_signs_reversed = (
                int(lane.get("id")) ^ int(connected_lane.get("id"))
            ) < 0

            # Get the reference line direction match
            reference_line_direction_reversed = _is_reference_line_direction_reversed(
                road, connected_road
            )
...
def _get_connected_lanes_with_roads(
    lane: etree._Element,
    road: etree._Element,
    junctions_id_map: dict[str, etree._Element],
    roads_id_map: dict[str, etree._Element],
    ...
    linkage_tag = (
        ...
        else models.LinkageTag.PREDECESSOR
    )

    # Get the road's linked road IDs
    linked_road_id = (
        utils.get_successor_road_id(road)
        if linkage_tag == models.LinkageTag.SUCCESSOR
        else utils.get_predecessor_road_id(road)
    )
    linked_road = roads_id_map.get(linked_road_id)

    # If the lane has a link element - add the connected lane
    if (
        linked_road is not None
        and (
            linked_lane := utils.get_lane_link_element(
                lane, linked_road_id, linkage_tag
            )
        )
        is not None
    ):
    ...
    if linked_junction is not None:
        ...
        junction_connections = [
            ...
            if jc.get("incomingRoad") == road.get("id")
        ]
        ...
        junction_connections_with_lane_links = [
            (
                ...
                [
                    ll
                    for ll in utils.get_lane_links_from_connection(jc)
                    if ll.get("from") == lane.get("id")
                ],
            )
            for jc in junction_connections
        ]
        ...
        for junction_connection_with_lane_links in junction_connections_with_lane_links:
            ...
            for lane_link in junction_connection_with_lane_links[1]:
                ...
                connected_road = roads_id_map.get(
                    int(junction_connection_with_lane_links[0].get("connectingRoad"))
                )
                ...
                if connected_lane_sections:
                    ...
                    connected_lanes = [
                        l
                        for l in utils.get_lanes(connected_lane_sections[0])
                        if l.get("id") == lane_link.get("to")
                    ]

                    if connected_lanes:
                        # We have a connected lane matching the linked id - add it with its connecting road
                        connected_lanes_with_roads.append(
                            (connected_lanes[0], connected_road)
                        )

    return connected_lanes_with_roads


def _is_reference_line_direction_reversed(
    road: etree._Element, connected_road: etree._Element
) -> bool:
    """
    ...
    connected_road_start, connected_road_end = (
        utils.get_start_point_xyz_from_road_reference_line(connected_road),
...
def _raise_issue(
    checker_data: models.CheckerData,
    lane: etree._Element,
    connected_lane: etree._Element,
    road: etree._Element,
    connected_road: etree._Element,
) -> None:
    """
